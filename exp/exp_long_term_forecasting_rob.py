...
class Exp_Long_Term_Forecast_rob(Exp_Basic):
    ...
    def test_noise(self, vali_data, vali_loader, criterion):
    ...
    def train(self, setting, pretrain_path=None):
        ...
        for epoch in range(30):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                pgd_attack = PGD(self.model, eps=self.args.eps, alpha=self.args.alpha, steps=self.args.steps, random_start=True, device=self.device, pred_len = self.args.pred_len)
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        adv_loss = loss_rob(outputs_PGD,outputs_nos) * self.args.adv_loss

                        loss_cls = criterion(outputs_PGD, batch_y) + IB_loss_PGD

                        sim_loss = loss_rob(outputs_th, outputs_nos) * self.args.sim_loss

                        loss_feature = loss_rob(feature_PGD, feature_nos) * self.args.feat_loss
            ...
            test_loss = self.test_noise(test_data, test_loader, criterion)
        ...
        best_model_path = os.path.join(path, 'rob_checkpoint.pth')
